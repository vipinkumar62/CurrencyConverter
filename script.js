const apiKey = 'c617ec2b378fa2ff27d74c8b'; 
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            const currencyOptions = Object.keys(data.conversion_rates)
                .map(currency => `<option value="${currency}">${currencyNames[currency] || currency}</option>`)
                .join('');
    ...
    document.getElementById('converter-form').addEventListener('submit', event => {
        ...
        fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/pair/${fromCurrency}/${toCurrency}/${amount}`)
            ...
            .then(data => {
                ...
                document.getElementById('result').innerText = `${amount} ${currencyNames[fromCurrency] || fromCurrency} = ${result} ${currencyNames[toCurrency] || toCurrency}`;
            })
